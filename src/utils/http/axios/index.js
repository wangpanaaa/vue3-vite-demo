  ...
  transformResponseHook: (res, options) => {
    ...
    if (hasSuccess) {
      ...
      if (isNull(successMsg) || isUnDef(successMsg) || isEmpty(successMsg)) {
        successMsg = api.operationSuccess
      }

      if (options.successMessageMode === 'modal') {
        createSuccessModal({ title: api.successTip, content: successMsg })
    ...
    throw new Error(timeoutMsg || api.apiRequestFailed)
  },
  ...
  responseInterceptorsCatch: (axiosInstance, error) => {
    ...
    try {
      if (code === 'ECONNABORTED' && message.indexOf('timeout') !== -1) {
        errMessage = api.apiTimeoutMessage
      }
      if (err?.includes('Network Error')) {
        errMessage = api.networkExceptionMsg
      }
